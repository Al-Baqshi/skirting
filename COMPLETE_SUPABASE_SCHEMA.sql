...
-- This creates all tables, functions, policies, and views for admin functionality
-- ============================================================================
...
DROP POLICY IF EXISTS "Allow admin read all products" ON public.products;
...
CREATE POLICY "Users can update own profile"
  ...
  WITH CHECK (
    auth.uid() = id
    AND (role = OLD.role)
    AND (is_active = OLD.is_active OR is_admin(auth.uid()))
  );
...
-- ============================================================================
-- Next steps:
-- 1. Create your first admin user in Supabase Dashboard → Authentication → Users → Add User
-- 2. Get the user's UUID or email
-- 3. Make them admin by running:
